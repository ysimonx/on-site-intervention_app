...
class _SchemaPageState extends State<SchemaPage> {
  ...
  final SignatureController _controllerSchema = SignatureController(
    penStrokeWidth: 1,
    penColor: Colors.red,
  ...
  void initState() {
    super.initState();
    _controllerSchema.addListener(() => log('Value changed'));
  }

  @override
  void dispose() {
    // IMPORTANT to dispose of the controller
    _controllerSchema.dispose();
    super.dispose();
  }

  Future<void> exportImage(BuildContext context) async {
    if (_controllerSchema.isEmpty) {
    ...
    final Uint8List? data =
        await _controllerSchema.toPngBytes(height: 1000, width: 1000);
    if (data == null) {
      return;
    }
  ...
  Future<void> exportSVG(BuildContext context) async {
    ...
    final String stringSVG = _controllerSchema.toRawSVG()!;
    if (!mounted) return;
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              IconButton(
                ...
                color: Colors.blue,
                onPressed: () {
                  setState(() => _controllerSchema.undo());
                },
                tooltip: 'Undo',
              ),
              IconButton(
                ...
                color: Colors.blue,
                onPressed: () {
                  setState(() => _controllerSchema.redo());
                },
                tooltip: 'Redo',
              ),
              //CLEAR CANVAS
              IconButton(
                ...
                color: Colors.blue,
                onPressed: () {
                  setState(() => _controllerSchema.clear());
                },
                tooltip: 'Clear',
              ),
